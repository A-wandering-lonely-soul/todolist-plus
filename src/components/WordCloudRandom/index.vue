...
class RandomAnimate {
  ...
  //根据元素的缩放值（scale）计算元素的 z 值。元素的 z 值与其缩放大小成正比（更大形成或更接近观察者的元素具有更高的层级）。元素的 z-index 属性被设置为其 z 值
  setScale(item: AnimateElement) {
    ...
    animateInfo.z = ~~(
      ((animateInfo.scale - minScale) / (1 - minScale)) *
      maxZ
    );
  }
  setPosition(item: AnimateElement) {
    ...
    if (animateInfo.x === undefined) {
      ...
      let { xInterval, yInterval, animateInterval } = this; // xInterval = xInterval - Math.random() * 15000 // yInterval = yInterval - Math.random() * 15000
  ...
  setHover(item: AnimateElement) {
    ...
    item.addEventListener('mouseleave', () => {
      (item.isPaused = false), (item.style.zIndex = item.animateInfo.z + '');
      item.style.transform = item.style.transform.replace(
        /scale((\d+.?\d*))/,
...
<template>
  <div class="word-cloud-container" ref="container"><slot></slot></div>
</template>
